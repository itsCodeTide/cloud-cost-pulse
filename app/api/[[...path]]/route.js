...
async function seedIfEmpty(db) {
  ...
  // Cost Data — last 8 months, per service
  const now = new Date()
  const costData = []
  for (let m = 7; m >= 0; m--) {
  ...
  await db.collection('recommendations').insertMany(recs.map(r => ({ id: uuidv4(), created_at: new Date(), ...r })))

  await db.collection('meta').insertOne({ _id: 'seed', done: true, at: new Date() })
}
  ...
  try {
    ...
    if (path === 'dashboard') {
      const [resources, costData, budgetDoc, recs] = await Promise.all([
        ...
        db.collection('cost_data').find({}).sort({ month_index: 1 }).toArray(),
        db.collection('budgets').findOne({}, { sort: { created_at: -1 } }),
      ...
      // active services (services with >0 cost this month)
    ...
    if (path === 'resources') {
      const items = await db.collection('resources').find({}).sort({ monthly_cost: -1 }).toArray()
      return ok(items.map(({ _id, ...r }) => r))
    }

    if (path === 'cost-data') {
      const items = await db.collection('cost_data').find({}).sort({ month_index: 1 }).toArray()
      return ok(items.map(({ _id, ...r }) => r))
    }

    if (path === 'recommendations') {
      const items = await db.collection('recommendations').find({}).sort({ potential_savings: -1 }).toArray()
      return ok(items.map(({ _id, ...r }) => r))
    }

    if (path === 'budget') {
      const b = await db.collection('budgets').findOne({}, { sort: { created_at: -1 } })
...
export async function POST(request, { params }) {
  try {
    ...
    if (path === 'budget') {
      const doc = { id: uuidv4(), monthly_budget: Number(body.monthly_budget) || 15000, created_at: new Date() }
    ...
    if (path === 'reset') {
      ...
      await db.collection('cost_data').deleteMany({})
