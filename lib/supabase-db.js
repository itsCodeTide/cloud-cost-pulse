...
const STORAGE_TABLES = {
  ...
  meta: 'workspace_meta', user_activity: 'user_activity',
}
const AUXILIARY_TABLE = 'workspace_auxiliary_data'
function storageTable(name) { return STORAGE_TABLES[name] || AUXILIARY_TABLE }
function hydrateRow(name, row) {
  const value = { ...(row.payload || {}), _id: row.payload?._id || row.record_id }
  if (row.tenant_id != null) value.tenantId = row.tenant_id
  const mappings = {
    users: ['email', 'full_name', 'role', 'last_seen_at'], tenants: ['name', 'plan'],
    resources: ['resource_name', 'service_type', 'region', 'status', 'monthly_cost'],
    cost_data: ['source', 'month', 'service', 'month_index', 'cost'],
    cost_history: ['month', 'service_type', 'service_key', 'month_index', 'cost'],
    budgets: ['name', 'monthly_budget', 'amount', 'period'],
    azure_connections: ['azure_tenant_id', 'azure_client_id', 'azure_subscription_id'],
    reports: ['title', 'report_type'], notifications: ['type', 'title', 'message', 'severity', 'read'],
    settings: ['currency', 'data_source'], recommendations: ['title', 'category', 'priority', 'status', 'estimated_savings'],
    audit_logs: ['user_id', 'action', 'entity', 'entity_id', 'prev_value', 'new_value'],
    user_activity: ['user_id', 'activity_type', 'route', 'method', 'ip_address', 'user_agent', 'metadata', 'occurred_at'],
  }
  for (const key of mappings[name] || []) {
    if (row[key] !== null && row[key] !== undefined) value[key === 'data_source' ? 'dataSource' : key] = row[key]
  }
  if (name === 'azure_connections') {
    if (row.azure_tenant_id != null) value.azureTenantId = row.azure_tenant_id
    if (row.azure_client_id != null) value.azureClientId = row.azure_client_id
    if (row.azure_subscription_id != null) value.azureSubscriptionId = row.azure_subscription_id
  }
  if (name === 'audit_logs' && row.user_id != null) value.userId = row.user_id
  if (name === 'user_activity' && row.user_id != null) value.userId = row.user_id
  return value
}
...
async function readCollection(name) {
  ...
  const promise = query.then(({ data, error }) => {
    ...
    return (data || []).map((row) => hydrateRow(name, row))
  })
...
async function writeDocument(name, doc) {
  ...
  if (name === 'user_activity') Object.assign(row, { user_id: normalized.userId || normalized.user_id || null, activity_type: normalized.activity_type || 'api_request', route: normalized.route || '', method: normalized.method || 'GET', ip_address: normalized.ip_address || null, user_agent: normalized.user_agent || null, metadata: normalized.metadata || {}, occurred_at: normalized.occurred_at || new Date().toISOString() })
  // Every storage table, including the auxiliary table, uses record_id as its
  // primary key. The old composite conflict target was not backed by a unique
  // constraint and caused Supabase to reject otherwise valid upserts.
  const { error } = await client.from(table).upsert(row, { onConflict: 'record_id' })
  if (error) {
