...
async function readCollection(name) {
  const { data, error } = await client.from('app_data').select('doc_id,data').eq('collection', name)
...
function collection(name) {
  return {
    ...
    async deleteOne(query = {}) { const row = (await readCollection(name)).find((item) => matches(item, query)); if (!row) return { deletedCount: 0 }; const { error } = await client.from('app_data').delete().eq('collection', name).eq('doc_id', String(row._id)); if (error) throw error; return { deletedCount: 1 } },
    async deleteMany(query = {}) { const rows = (await readCollection(name)).filter((row) => matches(row, query)); for (const row of rows) { const { error } = await client.from('app_data').delete().eq('collection', name).eq('doc_id', String(row._id)); if (error) throw error } return { deletedCount: rows.length } },
